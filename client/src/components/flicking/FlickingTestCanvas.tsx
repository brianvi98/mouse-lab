...
import {
  usePointerCapture,
  type PointerDataPoint,
} from "@/hooks/usePointerCapture";

type FlickingTestCanvasProps = {
  onCompletion: (data: PointerDataPoint[]) => void;
};

function FlickingTestCanvas({
  onCompletion = () => {},
}: FlickingTestCanvasProps) {
  ...
  return (
    <div
      ref={canvasRef}
      className="relative w-full h-full bg-canvas flex items-center justify-center outline-2"
    >
      {isRecording && (
        <h1 className="text-3xl absolute top-2 left-1/2 z-10">{timer}</h1>
      )}
      <div
        ref={ballRef}
        className="w-20 h-20 rounded-full bg-flick-orange absolute"
      ></div>
      ...
      {!isRecording && (
        ...
        >
          <div className="flex flex-col items-center">
            ...
            <p className="text-m">
              1. The target will teleport around every{" "}
              {TELEPORT_INTERVAL_MS / 1000} seconds.
